...
@interface WildCardLCAnimatedPageControl ()<UIScrollViewDelegate>
...
@implementation WildCardLCAnimatedPageControl
...
- (void)addIndicatorsWithIndex:(NSInteger)index{
    for (NSInteger number = index; number < _numberOfPages; number ++ ) {
        ...
        if (_pageStyle == LCFillColorPageStyle) {
            indicator = [[WildCardIndicatorView alloc] init];
            [(WildCardIndicatorView *)indicator backView].backgroundColor = _currentPageIndicatorColor;
            [(WildCardIndicatorView *)indicator frontView].backgroundColor = _pageIndicatorColor;
            [(WildCardIndicatorView *)indicator backView].layer.cornerRadius = _radius;
            [(WildCardIndicatorView *)indicator frontView].layer.cornerRadius = _radius;
            [self configZeroScaleAnimation:[(WildCardIndicatorView *)indicator backView]];
            [self configZeroScaleAnimation:[(WildCardIndicatorView *)indicator frontView]];
        }
        else{
...
- (void)configCurrentIndicator{
    ...
    if ([view isKindOfClass:[WildCardIndicatorView class]]) {
        [(WildCardIndicatorView *)view frontView].layer.timeOffset = 1.0f;
    }
    else{
        view.layer.timeOffset = 0.0f;
    }
    ...
    if (self.indicatorViews.count) {
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            if (_pageStyle == LCScaleColorPageStyle) {
                ...
                [(WildCardIndicatorView *)currentIndicator frontView].layer.timeOffset = 1.0f;
            }
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context{
    ...
    if (rate >= 0.0f && rate <= _numberOfPages - 1) {
        ...
        if (_pageStyle == LCScaleColorPageStyle) {
            ...
            [(WildCardIndicatorView *)currentPointView frontView].layer.timeOffset = timeOffset;
            [(WildCardIndicatorView *)lastPointView frontView].layer.timeOffset = 1.0f - timeOffset;
        }
        self.currentPage = currentIndex;
    }
}
